...
if($udetails)
{
	...
	assign('profilePic', $userquery->getUserThumb(false,false,$udetails["userid"]));
...
Assign('u_control', $profile);	
	...
	if ((isset($wowza)) and ($wowza instanceof CB_wowza)){
		...
		$application = $cblive->get_live_channels(array("userid"=>$udetails['userid']));
		if ($application){

			$application = $application[0];
			assign('application',$application);
			try{
				$live = $wowza->stream($application["channel_name"],$udetails["username"]);
				$live["live"] = "yes";
				$live["stream"] =$wowza->build_upstream_url($application["channel_name"],$udetails["username"],"hls");
				$live["extension"] = "m3u8";
				
				assign("live",$live);
			}catch(Exception $e){
				assign("error",strtoupper(name($udetails))." is not currently Live now ! ");
			}
		}
	}
}else{
