<?php
	...
	*/
	...
	// logData($outputFileName,'video_files');

	$sprite_count = $ffmpeg->sprite_count;
	$video_files = json_encode($ffmpeg->video_files);
	/*$db->update(tbl('video'), array("video_files","sprite_count"), array($video_files,$sprite_count), " file_name = '{$outputFileName}'");
	*/

	// getting extension to pass it to app
	// logData(VIDEOS_DIR.'/'.$file_directory.$outputFileName.'/'.$outputFileName.'.m3u8','video_files');
	// logData(VIDEOS_DIR.'/'.$file_directory.$outputFileName.'/'.$outputFileName.'.mpd','video_files');
	// logData(VIDEOS_DIR.'/'.$file_directory.$outputFileName.'.mp4','video_files');

	if(file_exists(VIDEOS_DIR.'/'.$file_directory.$outputFileName.'/'.$outputFileName.'.m3u8')){

		$file_type    = 2;

	}else if(file_exists(VIDEOS_DIR.'/'.$file_directory.$outputFileName.'/'.$outputFileName.'.mpd')){
	...
	/*$db->update(tbl('video'), array("file_type"), array($file_type), " file_name = '{$outputFileName}'");*/


	if (stristr(PHP_OS, 'WIN'))
	{
		exec(php_path()." -q ".BASEDIR."/actions/verify_converted_videos.php $orig_file $dosleep $file_type $sprite_count $video_files");
	}elseif(stristr(PHP_OS, 'darwin'))
	{
		exec(php_path()." -q ".BASEDIR."/actions/verify_converted_videos.php $orig_file $dosleep $file_type $sprite_count $video_files </dev/null >/dev/null &");
		
	} else {
		exec(php_path()." -q ".BASEDIR."/actions/verify_converted_videos.php $orig_file $dosleep $file_type $sprite_count $video_files &> /dev/null &");
	}

	
	$wm_filepath=explode('.', $orig_file);
	$orig_watermark=$wm_filepath[0].'-wm.'.$wm_filepath[1];

	//ulinking original files
	if(!isset($_GET['test'])){
	unlink($orig_file);
	if(file_exists($orig_watermark)){
	unlink($orig_watermark);
	}
	}
