...
switch($mode)
{
	...
	case "upload":
	{
		...
		if($config_for_mp4 == "yes" && $extension == "mp4" ) {
			...
			move_uploaded_file($tempFile,$orginal_file);
	
			$log = "";
	        $configs = array();
	        require_once(BASEDIR.'/includes/classes/conversion/ffmpeg.class.php');
	        $ffmpeg = new FFMpeg($configs, $log);
			$vid_file = $orginal_file;
	        $file_info = $ffmpeg->get_file_info($vid_file);
	        
	        $thumbs_res_settings_28 = thumbs_res_settings_28();
			$thumbs_num = config('num_thumbs');

			$thumbs_input['vid_file'] = $vid_file;
			$thumbs_input['num'] = $thumbs_num;
			$thumbs_input['duration'] = $file_info["duration"];
			
			$thumbs_input['file_directory'] = $file_directory;
			$thumbs_input['file_name'] = $file_name;
	      
	        foreach ($thumbs_res_settings_28 as $key => $thumbs_size) {
