...
class CBvideo extends CBCategory
{
	...
	function init()
	{
		...
		// removed 'vid_cat_height' and 'vid_cat_width' from code as it has no connection to db after cb 2.5.1
		/*		
		...
		*/
		...
		// checkmate
        $basic_fields = array(
            'videoid', 'videokey', 'userid', 'title','file_type','video_files','server_ip', 'description', 'tags', 'category','file_directory',
            'active', 'date_added', 'broadcast', 'rating','voter_ids', 'file_server_path', 'files_thumbs_path',
    ...
    function basic_fields_setup() {
        ...
        // checkmate
        $basic_fields = array(
            'videoid', 'videokey', 'userid', 'title','file_type','video_files', 'description', 'tags', 'category',
            'active', 'date_added', 'broadcast', 'rating','voter_ids', 'file_server_path', 'files_thumbs_path',
            'file_thumbs_count', 'duration', 'has_hq', 'has_mobile', 'views', 'file_name', 'rated_by',
	...
	function update_video($array=NULL)
	{
		...
		$max_title_len = $Cbucket->configs['max_video_title'];
		...
		if(empty($eh->error_list))
		{
			...
			}elseif(strlen($array['title']) > $max_title_len){
				e(lang("Title exceeds max length of ".$max_title_len." characters"));
			}else{
				//pr($upload_fields);	
				#pr($query_field,true);
		...
		if(is_array($files))
		{
			foreach($files as $file)
			{
				...
				{	
					...
					foreach($result as $result1)
					{
						// exit(pr($result1['file_type'],true));


						$str = '/'.$result1['file_directory'].'/';

						if($result1['file_type']==2){

							// HLS delete
							$segment_files = glob(VIDEOS_DIR.$str.$file.'/'.$file."/*");
							...
							if(is_array($segment_files))
								...
								$videos_files = glob(VIDEOS_DIR.$str.$file.'/'.$file."*");
						...
						elseif($result1['file_type']==1){

							// DASH delete
							// exit(pr($file,true));
							$segment_files = glob(VIDEOS_DIR.$str.$file.'/'.$file."_segments/"."*");
							...
							// exit(pr(VIDEOS_DIR.$str.$file,true));
