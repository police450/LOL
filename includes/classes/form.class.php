...
class formObj 
{
	...
	*/
	...
	function createDropDown($field,$multi=FALSE, $skipall = false)
	{
		...
		$ddFieldStart = '<select name="'.$field_name.'" id="'.$field['id'].'" class="'.$field['class'].'" '.$field['extra_tags'].'>';
		$arrayName = $this->rmBrackets($field['name']);
		if (!is_array($field['value'])) {
			$field['value'] = explode(",", $field['value']);
		}
		...
		$checked_fields = explode("#",$field['checked']);
		...
		{	
			...
			{	
				...
				foreach ($checked_countries as $keyy => $valuee) {
					# code...

				if($_POST[$arrayName] == $key || $valuee == $key)
				{	
			...
			if(!$value){
						$fieldOpts .='<option value="'.$key.'" '.$checked.' >'.'None'.'</option>';
			}else{
						$fieldOpts .='<option value="'.$key.'" '.$checked.' >'.$value.'</option>';	
			}
		}
		$ddFieldEnd = '</select>';
