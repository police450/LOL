...
class ClipBucket
{
  ...
  #  var $version = VERSION;
    ...
    var $on_delete_video = array();
     ...
     /**
     * This array contains
     * all video type running in cb
     */
    var $video_type = array('flv_mp4','dash','hls');
    var $active = false;
    var $license = "";
    var $license_local = "";
    ...
    function ClipBucket()
    {
        ...
        $this->license = $this->configs["cb_license"];
        $this->license_local = $this->configs["cb_license_local"];
    }
    ...
    function LatestAdminMenu()
    {
        ...
        if ($this->active){
            ...
            $NewMenu['ClipBucket']['License'] = 'update_license.php';
        }
    ...
    /**
     * Funtion used to get list of countries
     */
    ...
    /**
    * Funtion used to get blocked or unblocked list of countries
    */
    function get_isblocked_countries($type=0)
    {
        global $db;
        if($type==1 || $type==0){
            $results = $db->select(tbl("countries"), "*","is_blocked LIKE '".$type."'");
    ...
    /**
    * Funtion used to block countries
    */
    function geo_block_country($country_id)
    {
        global $db;
        $block=1;
        $results = $db->update(tbl("countries"), array('is_blocked'), array($block)," country_id LIKE '".$country_id."'");
        if($results){    
            return true;
        }else{
            return false;    
    ...
    * Funtion used to block countries
    */
    function geo_unblock_country($country_id)
    {
        global $db;
        $block=0;
        $results = $db->update(tbl("countries"), array('is_blocked'), array($block)," country_id LIKE '".$country_id."'");
    ...
    */
    ...
    /**
    * Funtion used to get client country
    */
    function get_client_country($ip)
    {
        $get_url='http://ip-api.com/json/'.$ip;
        $ch = curl_init();
    ...
    /**
    * Funtion used to check if client country is blocked or not
    */
    function check_blocked_country($country_code)
    {
       
        global $db;

        $results = $db->select(tbl("countries"), "is_blocked"," iso2 LIKE '".$country_code."'");
        $barray=array();
        foreach ($results as $result) {
    ...
    function head_menu($params = NULL)
    {
        ...
        $this->head_menu[] = array('name' => lang("Playlists"), 'icon' => '<i class="glyphicon glyphicon-film"></i>', 'link' => cblink(array('name' => 'playlists')), "this" => "playlists", "section" => "playlists");
...
//		
    ...
    function check_clipbucket_license($license,$localkey)
    {
    
        $results = $this->clipbucket_check_license($license,$localkey);
        $error_setting_link = '<a href="'.BASEURL.'/admin_area/update_license.php">Click Here to update ClipBucket License</a>';
       /* logData($results,"license");
        logData($_SERVER,"license");*/
        if(!$results)
        {
            if(!has_access("admin_access",TRUE) && !BACK_END){
                e($this->configs['closed_msg'],"w");
                template("global_header.html");
                template("message.html");
                exit();
            }else{
                e("Error while ClipBucket license - $error_setting_link","e");
            }
        }elseif ($results["status"]=="Invalid")
        {
            if(!has_access("admin_access",TRUE) && !BACK_END){
                ...
                e("Your ClipBucket License is Invalid - $error_setting_link","e");
            }
        ...
        }elseif ($results["status"]=="Expired")
        {
            if(!has_access("admin_access",TRUE) && !BACK_END){
                ...
                e("Your ClipBucket License is Expired - $error_setting_link","e");
            }
        ...
        }elseif($results["status"]=="Suspended")
        {
            if(!has_access("admin_access",TRUE) && !BACK_END){
                ...
                e("Your ClipBucket is suspended - $error_setting_link","e");
            }
    ...
    function clipbucket_check_license($licensekey,$localkey="")
    {
        //return array('status'=>'Active');
        $whmcsurl = "http://client.clip-bucket.com/";
        $prefix = "CBCORP";
        $licensing_secret_key = "CBCORP"; # Set to unique value of chars
        ...
        if (!$localkeyvalid) {
            ...
            if ($results["status"]=="Active") {
                ...
                global $db;
                $db->update(tbl("config"),array("value"),array($results["localkey"]),"name='cb_license_local'") ;
            }
