<?php
    ...
    */
    ...
    function video_playable($id) {
        ...
        global $cbvideo,$userquery,$Cbucket;

        //getting required attributed for geo blocking
        //getting client ip
        $c_ip=$Cbucket->get_client_ip();
        // $c_ip='43.245.9.236';
        //getting client country on the base of ip
        $c_country_code=$Cbucket->get_client_country($c_ip);
        // checking if country is blocked or not
        // $c_check_blocked_country=$Cbucket->check_blocked_country($c_country_code);
        ...
        // this code is used for exploding multiple blocked countries
        $blocked_fields = explode("#",$vdo['blocked_countries']);
        $blocked_countries=array();
        foreach ($blocked_fields as $key => $value) {
            # code...
            if($value == ' '){
                // do nothing
            }elseif($value == ''){
        ...
        // this code is used for exploding multiple blocked countries
        if(!empty($c_country_code)){

           foreach ($blocked_countries as $key => $value) {
                   # code...
               if($value==$c_country_code){
               ...
               // code for chacking country ended
    }
    ...
    elseif ($check_mate == 1) {
        e(lang("geo_restriction_message"));
        return false;
       ...
       if(is_array($vid_thumbs))
        foreach($vid_thumbs as $thumb)
        {
            if(file_exists($thumb) && filesize($thumb)>0)
            {
                ...
                if ($multi && $size != 'original'){
                    if(strstr($thumb,$size)){
                        if($return_full_path){
                            $multi_thumbs[] = THUMBS_URL.'/'. $thumbDir . $thumb_file;
                        }
                        else{
                            $multi_thumbs[] = $thumb_file;
                        }
                    }
    ...
    */
    ...
    function get_video_file($vdetails,$return_default=true,$with_path=true,$multi=false,$count_only=false,$hq=false)
    {
        ...
        // checkmate 
        // upgraded for api corporate
                if(isset($vdetails['file_name'])){    
                    

                    if(file_exists(VIDEOS_DIR.'/' . $fileDirectory. $vdetails['file_name'].'/'.$vdetails['file_name'].".m3u8")){

                        $files[]    = VIDEOS_URL.'/' . $fileDirectory. $vdetails['file_name'].'/'.$vdetails['file_name'].".m3u8" ;

                    }else if(file_exists(VIDEOS_DIR.'/' . $fileDirectory. $vdetails['file_name'].'/'.$vdetails['file_name'].".mpd")){

                     $files[]    = VIDEOS_URL.'/' . $fileDirectory. $vdetails['file_name'].'/'.$vdetails['file_name'].".mpd";   

                 }else if(file_exists(VIDEOS_DIR.'/' . $fileDirectory. $vdetails['file_name'].".mp4")){

                     $files[]    = VIDEOS_URL.'/' . $fileDirectory. $vdetails['file_name'].".mp4";   

                 }else{
                     
                     $dir_files[]    = glob(VIDEOS_DIR.'/' . $fileDirectory. $vdetails['file_name']."*");

                     foreach ($dir_files[0] as $key => $value) {

                        $files[] = str_replace(BASEDIR,BASEURL,$value);
    ...
    function update_processed_video($file_array,$status='Successful',$ingore_file_status=false,$failed_status='')
    {
        ...
        //logData($file_array,"argv_new");
        $video_files = $file_array["video_files"];
        $sprite_count = $file_array["sprite_count"];
        ...
        if(file_exists($file_path) && $file_size>0 && !$ingore_file_status)
        {
            ...
            $db->update(tbl("video"),array("status","duration","failed_reason","video_files","sprite_count","file_type"),
                array($status,$duration,$failed_status,$video_files,$sprite_count,$file_type)," file_name='".$file_name."'");
        }else
        {
            //$duration = $stats['output_duration'];
            //if(!$duration)
    ...
    function get_video_files($vdetails,$return_default=true,$with_path=true,$multi=false,$count_only=false,$hq=false){
       ...
       if(is_array($Cbucket->custom_video_file_funcs))
        foreach($Cbucket->custom_video_file_funcs as $func)
         ...
         #Now there is no function so lets continue as
        //Commented the resolutions code for dashed files
        //  if(isset($vdetails['file_name'])){
        ...
        //         pr($video_file,true);
                ...
                // pr(VIDEO_VERSION,true);

            if($with_path){
                if(VIDEO_VERSION == '2.7'){
                            // $files[]    = VIDEOS_URL.'/' . $fileDirectory. $vdetails['file_name']."-360.mp4" ;
                    if(file_exists(VIDEOS_DIR.'/' . $fileDirectory. $vdetails['file_name'].'/'.$vdetails['file_name'].".m3u8")){
                ...
                if(file_exists(VIDEOS_DIR.'/' . $video_file.'/'.$vdetails['file_name'].".m3u8")){

                    $files[]    = VIDEOS_URL.'/' . $video_file.'/'.$vdetails['file_name'].".m3u8" ;

                }else if(VIDEOS_DIR.'/' . $video_file.'/'.$vdetails['file_name'].".mpd"){

                    $files[]    = VIDEOS_URL.'/' . $video_file.'/'.$vdetails['file_name'].".mpd" ;  

                }else if(VIDEOS_DIR.'/' . $video_file.'/'.$vdetails['file_name'].".mp4"){

                    $files[]    = VIDEOS_URL.'/' . $video_file.'/'.$vdetails['file_name'].".mp4" ;  

                }else{

                    $dir_files[]    = glob(VIDEOS_DIR.'/' . $video_file.'/'.$vdetails['file_name']."*");
 ...
 function get_downloadable_files($vdetails,$return_default=true,$with_path=true,$multi=false,$count_only=false,$hq=false){
   ...
   if(is_array($Cbucket->custom_downloadable_file_funcs))
    foreach($Cbucket->custom_downloadable_file_funcs as $func)
        if(function_exists($func))
        {
            $func_returned = $func($vdetails, $hq);
            if($func_returned)
        ...
        if(isset($vdetails['file_name'])){
            if(VIDEO_VERSION == '2.7'){
                ...
                $vid_files = glob(VIDEOS_DIR."/".$fileDirectory . $vdetails['file_name']."/".$vdetails['file_name']."*");
                ...
                if($with_path){
                    if(VIDEO_VERSION == '2.7'){
                     $files[]    = VIDEOS_URL.'/' . $fileDirectory. $vdetails['file_name'] .'/' .$video_file;
                        // $files[]    = VIDEOS_URL.'/' . $fileDirectory. $vdetails['file_name']."/".$vdetails['file_name'];
                 }else if(VIDEO_VERSION == '2.6'){
 ...
 function upload_thumb($array)
 {
    ...
    */
    ...
    function get_video_sprite($video) {
        try {
            ...
            $multiserver_path = $video['file_server_path'];

            $file = SPRITES_DIR.'/'.$file_directory.'/'.$filename.'-sprite.png';
            

            if(!empty($multiserver_path)){

                $file = $multiserver_path.'/sprites/'.$filename.'-sprite.png';
                $response['file']  = $file;
    ...
    */
    ...
    /**
    * Used to make connection with mongo views collection
    * @since : 13 Nov, 2017
    */

    function mongoConViews()
    {
        # code...
        require ''.BASEDIR.'/includes/mongo/vendor/autoload.php';
        
        $client = new MongoDB\Client("mongodb://localhost:27017");
        $collection = $client->cb_mongo->cb_earning_views;
        return $collection;
    }


    /**
    * this is parent function getting all params and passing to add_mongo_view()
    * @since : 13 Nov, 2017
    */

    function add_video_views($params)
    {   
        
        global $Cbucket;

        if($params['videoid'] && $params['userid']){

            $client_ip=$Cbucket->get_client_ip();
            $client_country_code=$Cbucket->get_client_country($client_ip);
            
            if(!$client_ip){
                
                throw new Exception("Client ip is empty");
                
            }elseif(!$client_country_code){

                throw new Exception("Client country code is empty");
                
            }else{

                $response = add_mongo_view($params,$client_ip,$client_country_code);
                ...
                if($response){
                    ...
                    throw new Exception("View response is empty view not added");
                }
            }
        }else{

            throw new Exception("Videoid or Userid is empty!");
    ...
    /**
    * returns current time object according to mongo format
    * @since : 13 Nov, 2017
    */
    ...
    function mongoNow()
    {
        ...
        $temp_date_time = new DateTime($today,$gmtTimezone);
            // $datetime = new MongoDB\BSON\UTCDateTime((new DateTime($today,$gmtTimezone))->getTimestamp()*1000);
        $datetime = new MongoDB\BSON\UTCDateTime($temp_date_time);

        return $datetime;
    }

    /**
    * Used to add video views to into Mongodb
    * @since : 13 Nov, 2017
    */

    function add_mongo_view($params,$client_ip,$client_country_code)
    {
     
        $collection = mongoConViews();        

        $view_result = $collection->findOne(
            [   
                'client_ip' => $client_ip , 
        ...
        if($view_result['_id']){
            ...
            $view_datetime = $view_result->time->toDateTime()->format('Y-m-d H:i:s');
            
            $current_datetime = date('Y-m-d H:i:s');

            $difference = round((strtotime($current_datetime) - strtotime($view_datetime))/3600, 1);
            
            // $datetime1 = new DateTime($view_datetime);
            // $datetime2 = new DateTime($current_datetime);
            // $interval = $datetime1->diff($datetime2);
            // // $difference = $interval->format('%y years %m months %a days %h hours %i minutes %s seconds');
            // $difference = $interval->format('%h');
            
            if($difference >= 1){

             $response = insert_view($params,$client_ip,$client_country_code);
             
         }else{
            
             $response = "View can't be added hour difference is (".$difference.") hours Searched view id = (".$view_result['_id'].") and time (".$view_datetime.") and now (".$current_datetime.")";
         }
        ...
        $r=insert_view($params,$client_ip,$client_country_code);
        return $r;   
    ...
    /**
    * Used to insert video views to into Mongodb views collection
    * @since : 13 Nov, 2017
    */


    function insert_view($params,$client_ip,$client_country_code)
    {
        # code...
       $collection = mongoConViews();

       $result = $collection->insertOne( ['userid' => (int)$params['userid'], 'videoid' => (int)$params['videoid'], 'client_ip' => $client_ip, 'country_code' => $client_country_code,'time' => mongoNow(),'flag' => (int)0,'view_status' => (int)0]);
       
       $response = "Inserted the video view with user id ".$params['userid']." and video id ".$params['videoid']." ip = ".$client_ip." country code = ".$client_country_code." recorded at time = ".mongoNow()." record inserted id : ".$result->getInsertedId();
