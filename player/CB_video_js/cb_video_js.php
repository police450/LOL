...
if (!function_exists('cb_video_js'))
{
	...
	function cb_video_js($in)
	{
		...
		$video_sprite = get_video_sprite($vdetails);
		if ($video_sprite['count']>0){
			assign('video_sprites','yes');
			assign('sprite_file',$video_sprite['file']);
			assign('sprite_count',$video_sprite['count']);
		}
		...
		switch ($function) {
			...
			case 'get_ultimate_ads':{
				if (CB_ULTIMATE_ADS == 'installed'){
					...
					global $CbUads;
					$ads_array = array("filter_ad"=>true,"status"=>"1","non_expiry"=>'true',"video"=>$video);
