...
//Using Multple Action
if(isset($_POST['make_verified_selected'])){
	for($id=0;$id<=count($_POST['check_user']);$id++){
		$userquery->action('verified',$_POST['check_user'][$id]);
	}
	$eh->flush();
	e("Selected users have been set as verified","m");
}
if(isset($_POST['make_unverified_selected'])){
	for($id=0;$id<=count($_POST['check_user']);$id++){
		$userquery->action('unverified',$_POST['check_user'][$id]);
	}
	$eh->flush();
	e("Selected users have been removed from verified list","m");
}
