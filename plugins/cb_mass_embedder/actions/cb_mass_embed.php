...
//Gett List of pending videos
	...
	if($embedType=='manual')
	{
		...
		$api = $_POST['maual_api'];
		include_once(PLUG_DIR."/".CB_MASS_EMBED_LOC."/apis/".$api.".api.php");
		...
		#Template(PLUG_DIR."/".CB_MASS_EMBED_LOC."/blocks/temp_videos.html",false);
		Template(PLUG_DIR."/".CB_MASS_EMBED_LOC."/blocks/temp_videos.html",false);
	...
	if($embedType=='keywords')
	{
		...
		if($cb_mass_embed->license_status)
		{
			...
			foreach($apis as $api)
			{
				...
				//Setting Keywords
				$embed->keywords = $configs['keywords'];
				...
				$msg.= "<div style='font-size:14px'><strong>".$embed->results."</strong> From <strong>$api</strong> - <strong>".$embed->results." </strong> Results found </div>";
			...
			// currently working 
			$vids = $embed->parse_and_get_results();

			$newResutls = array();
			if($vids){
				foreach($vids as $result){
					//Setting Categories
					$cat = $cbvid->get_default_category();
					//Matching Category
					if($embed->categorization=='auto')
						$thecat = $embed->match_cat($result['tags']);
					if($thecat)
						$result['category'] = $thecat;
					else	
						$result['category'][] = $cat['category_id'];
					if($embed->categorization=='selected')
					{
						$result['category'] = array();
						$result['category'][] = $embed->set_category;	
					}
			...
			assign("videos",$newResutls);
			assign("EMBED_PLUG_DIR",PLUG_DIR."/".CB_MASS_EMBED_LOC."/blocks");
			Template(PLUG_DIR."/".CB_MASS_EMBED_LOC."/blocks/videos.html",false);
	...
	if($embedType=='links')
	{
		...
		$input = str_replace("https://www.youtube.com/watch?v", "", $input);
		$input = explode("=", $input);
		...
		//Gett List of pending videos
		$my_test_api = new youtube();
		$input = array_filter($input);
		$vids = $my_test_api->get_details_from_url($input);
		...
		if($vids){
			foreach($vids as $result){
				...
				$cat = $cbvid->get_default_category();
				//Matching Category
				if($cb_mass_embed->categorization=='auto')
					$thecat = $cb_mass_embed->match_cat($result['tags']);
				if($thecat)
					$result['category'] = $thecat;
				else	
					$result['category'][] = $cat['category_id'];
				if($cb_mass_embed->categorization=='selected')
				{
					$result['category'] = array();
					$result['category'][] = $cb_mass_embed->set_category;	
				}
	...
	if($embedType=='yt_user')
	{
		...
		include_once(PLUG_DIR."/".CB_MASS_EMBED_LOC."/apis/youtube.api.php");
		...
		$vids = $embed->get_data_from_user($_POST['yt_user']);
		assign("videos",$vids);
		assign("EMBED_PLUG_DIR",PLUG_DIR."/".CB_MASS_EMBED_LOC."/blocks");
		Template(PLUG_DIR."/".CB_MASS_EMBED_LOC."/blocks/videos.html",false);
		...
		$vids = $embed->get_details_from_url();
...
<script type="text/javascript">
	$("#mass_embed").on("click", function(e){
		alert("hello worldo");
	});
</script>