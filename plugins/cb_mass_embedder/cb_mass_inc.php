...
//Function used to check CB brand removal license
	function check_cb_mass_embed_license($license,$localkey)
	{
		$results = mass_embed_check_license($license,$localkey);
		$error_setting_link = '<a href="'.BASEURL.'/admin_area/plugin.php?folder='.CB_MASS_EMBED_LOC.'/admin&file=cb_mass_configuration.php">Click Here to edit Mass Embed Settings</a>';
		if(!$results)
		{
			if(BACK_END)
			e("Error while loading CB Mass Embedder license - $error_setting_link","w");
		}
		elseif ($results["status"]=="Invalid")
		{
			if(BACK_END)
			e("Your CB Mass Embedder License is Invalid - $error_setting_link","w");
		}
		elseif ($results["status"]=="Expired")
		{
			if(BACK_END)
			e("Your CB Mass Embedder License is Expired - $error_setting_link","w");
		}
		elseif($results["status"]=="Suspended")
		{
			if(BACK_END)
			e("Your CB Mass Embedder is suspended - $error_setting_link","w");
		}
	...
	//Function used to check CB brand removal license
	function mass_embed_check_license($licensekey,$localkey="")
	{
		//return array('status'=>'Active');
		$whmcsurl = "http://client.clip-bucket.com/";
		$prefix = "CBMEPRO";
		$licensing_secret_key = "CBMEPRO"; # Set to unique value of chars
		...
		if (!$localkeyvalid) {
			...
			if ($results["status"]=="Active") {
				...
				global $db;
				$db->update(tbl("mass_embed_configs"),array("value"),array($results["localkey"]),"config_id=8") ;
			}
	...
	function get_cb_mass_configs()
	{
      global $db;
      $results = $db->select(tbl("mass_embed_configs"),"*");
      $configs  = array();
	  foreach($results as $result)
	  {
	  	$configs[$result['config_name']] = $result['name'] = $result['config_value'];
	  }
    ...
    */
    ...
    function messUpLastChecked_mass($status) {
        $dateStamp = dateStamp();
    ...
    */
    ...
    function cleanUpLastChecked_mass($status, $lastChecked) {
        $alphabets = range('a', 'z');
    ...
    */
    ...
    function liscCheckLatest_mass() {
        ...
        if ((int) trim($success_ip) == (int) trim($current_ip)) {
           return array('status'=>'Active');
            ...
            $result = check_cb_mass_embed_license($lisc_key,$local_key);
            if ($result["status"] == 'Active') {
                global $db;
                $db->update(tbl('mass_embed_configs'),array('value'),array($current_ip),"name ='success_ip'");
                return $result;
            }
        }
    }
	...
	add_admin_menu("Mass Embedder","Mass Embed Videos",'cb_mass_embed.php',CB_MASS_EMBED_LOC.'/admin');
	add_admin_menu("Mass Embedder","Pending Videos",'pending_videos.php',CB_MASS_EMBED_LOC.'/admin');
	add_admin_menu("Mass Embedder","Mass Embed Table",'mass_embed_table.php',CB_MASS_EMBED_LOC.'/admin');
