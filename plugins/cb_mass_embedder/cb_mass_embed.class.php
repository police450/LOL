...
class cb_mass_embed
{
	...
	{
		$snatching_file = file_get_contents($snatching_file);
	...
	function grab_the_thumb($url,$saveto)
	{
	    $ch = curl_init ($url);
	 ...
	 */
	function download_and_insert_data($thumb_link)
	{
		...
		foreach($results as $result)
		{
			...
			foreach($result['thumbs'] as $key=>$thumb)
			{
				...
				if($key!=='big')
				{
					$this->snatch_it(urlencode($thumb_link),THUMBS_DIR,$file_key."-$count.jpg");
				}
				else
				{
					$this->snatch_it(urlencode($thumb_link),THUMBS_DIR,$file_key."-big.jpg");
				}
				$count++;
			}
